...
// ALUNO FAZER PALAVRAS ALEATORIAS
...
async function getPalavras(){
	const txtPalavras="palavras.txt";
	const request = await fetch (txtPalavras);
	const resposta = await request.text();
	lstPalavras=resposta.split('\r\n');
	console.log(lstPalavras);
}
function aleatoria(){
	const tamPalavras=lstPalavras.length;
	const rndIndex= Math.floor(Math.random()*tamPalavras);
	console.log(rndIndex)

	word = lstPalavras[rndIndex];
	console.log(word);
